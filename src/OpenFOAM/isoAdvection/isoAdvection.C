...
:
    ...
    alpha1_(alpha1),
    alpha1In_(alpha1.oldTime().nCnstIntFieldRef()), // Need to reorganise
    phi_(phi),
    U_(U),
    dVf_
    (
...
void Foam::isoAdvection::timeIntegratedFlux()
{
    ...
    scalarField& dVfIn = dVf_.nCnstIntFieldRef();
...
//                          << endl;
    ...
    surfaceScalarField::GeometricBoundaryField& dVfb = dVf_.nCnstBndFldRef();
...
//          Pout << "Recieved at time = " << mesh_.time().value() << ": dVfPatch = " << nbrdVfs << endl;
            ...
            // Combine fluxes
            scalarField& localFlux = dVf.nCnstBndFldRef()[patchI];
/*
