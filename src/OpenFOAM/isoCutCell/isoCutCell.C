...
// * * * * * * * * * * * *Macros for version invariance* * * * * * * * * * * //

#ifdef newAPI
#define nCnstIntFieldRef() ref()
#define nCnstBndFldRef() boundaryFieldRef()
#define GeometricBoundaryField Boundary
#else
#define nCnstIntFieldRef() internalField()
#define nCnstBndFldRef() boundaryField()
#endif
...
)
{
    ...
    scalarField& alphaIn = alpha1.nCnstIntFieldRef();
    forAll(alphaIn, ci)
    {
    ...
    forAll(mesh_.boundary(), patchI)
    {
        if (mesh_.boundary()[patchI].size() > 0)
        {
            ...
            fvPatchScalarField& alphaBP = alpha1.nCnstBndFldRef()[patchI];            
